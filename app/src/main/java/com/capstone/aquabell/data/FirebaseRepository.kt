...
class FirebaseRepository(
    ...
    fun listenToAlerts(): kotlinx.coroutines.flow.Flow<List<com.capstone.aquabell.data.model.AlertEntry>> = callbackFlow {
        try {
            ...
            val listenerRegistration = alertsCollection()
    ...
    suspend fun deleteResolvedAlerts() {
        try {
            ensureAuth()
            val querySnapshot = alertsCollection()
                .whereEqualTo("acknowledged", true)
                .get()
                .await()
            ...
            Log.e(TAG, "deleteResolvedAlerts failed: ${e.message}", e)
            throw e
        }
    }
    ...
    suspend fun acknowledgeAlert(alertId: String) {
        try {
            ensureAuth()
            alertsCollection().document(alertId).update("acknowledged", true).await()
        } catch (e: Exception) {
